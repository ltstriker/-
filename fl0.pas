...
procedure getsym;
  ...
  begin
    ...
    begin
      if eof(input) then
      begin
        write('PROGRAM INCOMPLETE');// writeln;exit;
      end;
...
begin
  if cx > cxmax then 
  begin write('PROGRAM TOO LONG'); writeln;exit;
  end;
  with code[cx] do
  begin  f := x;  l := y;  a := z
  end;
...
procedure  block(lev, tx : integer; fsys : symset);
  ...
  procedure  statement(fsys : symset);
    ...
    begin  getsym;
      if sym = ident then
      begin  enter(procedure1);  getsym  end
      else error(4);
      if sym = semicolon then getsym else error(5);
...
begin  {主程序}
  ...
  ssym['='] := eql;       ssym[','] := comma;
  ssym['.'] := period;     ssym['!'] := neq; {! for ≠}
  ssym['<'] := lss;       ssym['>'] := gtr;
  ssym['$'] := leq;      ssym['^'] := geq;  {use $ for leq    ^ for geq}
  ssym[';'] := semicolon;
